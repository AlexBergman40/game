...
void generatePossibleMoves::generateForWhitePawn()
{
  ...
  if (square & fifthRank)
  {
    ...
    if (right & ~leftMostColumn 
      && right & ~moveHistory[moveHistory.size()-2][BLACKPAWN]
      && (right >> 16) & ~moveHistory.back()[BLACKPAWN]
      && (right >> 16) & moveHistory[moveHistory.size()-2][BLACKPAWN]
      && right & moveHistory.back()[BLACKPAWN])
    {
      std::cout << "EN PASSANT" << std::endl;
      possibleMoves |= (right >> 8);
    }
    // check left => check for previous 2 square move
    if (left & ~rightMostColumn 
      && left & ~moveHistory[moveHistory.size()-2][BLACKPAWN]
      && (left >> 16) & ~moveHistory.back()[BLACKPAWN]
      && (left >> 16) & moveHistory[moveHistory.size()-2][BLACKPAWN]
      && left & moveHistory.back()[BLACKPAWN])
    {
      std::cout << "EN PASSANT" << std::endl;
...
{
  std::cout << "generating for a white knight" << std::endl;
}
...
{
  std::cout << "generating for a white king" << std::endl;
}
...
void generatePossibleMoves::generateForBlackPawn()
{
  ...
  if (square & fourthRank)
  {
    ...
    uint64_t left = square >> 1;
    if (right & ~leftMostColumn 
      && right & ~moveHistory[moveHistory.size()-2][WHITEPAWN]
      && (right << 16) & ~moveHistory.back()[WHITEPAWN]
      && (right << 16) & moveHistory[moveHistory.size()-2][WHITEPAWN]
      && right & moveHistory.back()[WHITEPAWN])
    {
      std::cout << "EN PASSANT" << std::endl;
      possibleMoves |= (right << 8);
    }
    // check left => check for previous 2 square move
    if (left & ~rightMostColumn 
      && left & ~moveHistory[moveHistory.size()-2][WHITEPAWN]
      && (left << 16) & ~moveHistory.back()[WHITEPAWN]
      && (left << 16) & moveHistory[moveHistory.size()-2][WHITEPAWN]
      && left & moveHistory.back()[WHITEPAWN])
    {
      std::cout << "EN PASSANT" << std::endl;
      possibleMoves |= (left << 8);
    }
